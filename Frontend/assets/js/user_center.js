...
$('#changeEmailBtn').click(function () {
    ...
    if (newEmailVal == currentUserEmail){
        ...
        $.ajax({
            url: "http://111.230.253.94:8081/user/updateEmail?token="+login_token+"&newEmail="+newEmailVal, // 后端 API 地址
            method: "POST", // 请求类型
            dataType: "json", // 返回的数据类型
            success: function (data) {
                if (data.code == 0) {
...
$('#btnOkVerify').click(function () {
    ...
    $.ajax({
        url: "http://111.230.253.94:8081/user/verifyEmailUpdate?token="+login_token+"&newEmail="+newEmailVal+"&emailCaptcha="+EmailCodeVal, // 后端 API 地址
        method: "POST", // 请求类型
        dataType: "json", // 返回的数据类型
        success: function (data) {
            console.log(data);
...
$('#changePasswordBtn').click(function () {
    window.location.href = "http://111.230.253.94/reset.html"
})
$('#changeUserInfoBtn').click(function () {
    window.location.href = "http://111.230.253.94/change_user_info.html"
})
$('#titleText').click(function () {
    window.location.href = "http://111.230.253.94/index.html"
})
    ...
    if (file) {
        ...
        compressImg(file, 0.2).then(r => {
            fileToBase64(r.file).then(result => {
                ...
                console.log(result)
                $.ajax({
                    url: basePath + "/user/uploadAvatar?token=" + login_token,
                    type: "post",
                    data: result,
